...
import { useLetterTrace } from '../../hooks/gameLogic/useLetterTrace';
    ...
    <View
      style={[
        styles.traceLine,
        {
          ...
          width: length,
          height: thickness,
          backgroundColor: traced ? '#8B5CF6' : '#CBD5E1',
...
export function LetterTraceScreen({ navigation }) {
  const { template, activeIndex, score, feedback, showConfetti, handleTraceStart, handleTraceMove, handleTraceEnd, speak } = useLetterTrace();
    ...
    <Screen>
      ...
      <View style={styles.promptRow}>
        <LinearGradient colors={['#FFFFFF', '#F5F3FF']} style={styles.promptCard}>
          <Text style={styles.promptEyebrow}>Trace the letter</Text>
          <Text style={styles.promptTitle}>Trace {template.letter}</Text>
          <Text style={styles.promptBody}>Start on the green dot and glide to the orange dot.</Text>
        </LinearGradient>

        <Pressable onPress={() => speak(`Trace the letter ${template.letter}`, 'question')} style={styles.soundButton}>
          <LinearGradient colors={['#8B5CF6', '#A78BFA']} style={styles.soundFill}>
      ...
      <LinearGradient colors={['#F5F3FF', '#EDE9FE']} style={styles.stageCard}>
        ...
        <View style={styles.boardWrap}>
          ...
          >
            <Text style={[styles.bigLetter, { fontSize: boardSize * 0.68 }]}>{template.letter}</Text>
                ...
                <View
                  ...
                  style={[
                    styles.traceDot,
                    {
                      left: point.x - 14,
                      top: point.y - 14,
                      backgroundColor: isStart ? '#22C55E' : isEnd ? '#F97316' : isTraced ? '#8B5CF6' : '#FFFFFF',
                      borderColor: isTraced ? '#6D28D9' : '#94A3B8'
                    }
                  ]}
                />
              );
        ...
        <View style={styles.footerHint}>
          <Text style={styles.footerHintText}>If you slip off the path, lift your finger and start again from the last dot.</Text>
        </View>
      </LinearGradient>
    </Screen>
  );
